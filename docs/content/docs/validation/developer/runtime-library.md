...
The default `validate()` method sits between the two: it probes the builder's current
content in place — running the same constraint checks as `build()`, but returning the
violations instead of throwing, and without handing back a possibly invalid message. It is
the builder-side counterpart to `ValidatableMessage.validate()`, and returns an empty list
when the content is valid. A product that does not implement `ValidatableMessage` supports
no constraints, so it is likewise treated as valid.

The `vBuild()` method is left deprecated for backward compatibility with the code
generated by Validation 1.x.
