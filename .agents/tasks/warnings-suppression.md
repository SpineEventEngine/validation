...
- [x] **Tests (unit):** `UnsignedIntegerWarningsSpec` already covers the
      gating contract used by `BoundedFieldGenerator` — when
      `setEnabled(false)` is in effect, `report()` is a no-op. That is the
      hook the renderer drives from `ValidationWarnings.unsignedFields`, so
      the unit-side success criterion is met.
- [x] **Tests (unit, DSL surface):** added
      `gradle-plugin/src/test/kotlin/io/spine/tools/validation/gradle/ValidationExtensionSpec.kt`,
      a `ProjectBuilder`-based suite covering the new
      `validation.java.warnings.unsignedFields` DSL surface — defaults to
      `true`, can be suppressed via direct property access, and via the
      nested `java { warnings { ... } }` `Action`-block syntax. The same
      tests verify that the resulting DSL value round-trips into a typed
      `ValidationWarnings` proto built by the plugin's
      `writeValidationWarningsSettings()` helper.
- [x] **Tests (integration, during compilation):** added
      `context-tests/src/test/kotlin/io/spine/tools/validation/UnsignedFieldWarningSpec.kt`
      — drives the real Spine Compiler `Pipeline` (via `PipelineSetup`
      from `compiler-testlib`) with `JavaValidationPlugin` against a new
      `uint32 + (range)` proto fixture
      (`context-tests/src/testFixtures/proto/spine/validation/unsigned_field_warning_spec.proto`).
      The test writes `ValidationWarnings` to the pipeline's
      `SettingsDirectory` (using the same consumer ID and `ProtoBinary`
      format as the Gradle plugin) and captures pipeline stdout with
      `tapConsole`. Three cases: warning *present* when no settings are
      written; warning *present* when `unsignedFields = true`; warning
      *absent* when `unsignedFields = false`.
...
- [x] **Version bump:** bumped to `2.0.0-SNAPSHOT.436` (commit
      `804f4d96cd`); dependency reports refreshed in commit `1d8ebc2536`.
- [x] **Verification:** `./gradlew clean build` is green on the branch
      after every change in this task (137 + 32 + 224 + 447 + 15 tests).
...
- 2026-05-21 — version bumped to `2.0.0-SNAPSHOT.436`; dependency reports
  regenerated by `./gradlew clean build`.
- 2026-05-21 — added integration tests for the new DSL: created a
  `:gradle-plugin` test sourceset (the module had none) and a
  `ValidationExtensionSpec` using Gradle's `ProjectBuilder` to instantiate
  the extension via `ObjectFactory` and exercise the
  `validation.java.warnings.unsignedFields` surface (default, direct set,
  nested `Action` block, and round-trip into a typed `ValidationWarnings`
  proto matching what the plugin writes). All 6 tests pass; full
  `./gradlew build` green.
- 2026-05-21 — reclassified the `:gradle-plugin` spec as a *unit* test
  (it only exercises the DSL surface). Added a proper **integration**
  test in `:context-tests`:
  `UnsignedFieldWarningSpec` drives the real Spine Compiler `Pipeline`
  via `PipelineSetup.byResources` with `JavaValidationPlugin`, against a
  new `uint32 + (range)` proto fixture, and asserts on captured pipeline
  stdout (`tapConsole`). Three cases — no settings, `unsignedFields = true`,
  `unsignedFields = false` — all pass; `./gradlew build` green.
